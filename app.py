# app.py
# Caption worker (Flask) — safe /enqueue with atomic job claim and VTT output.
...
# Caption worker (Flask) — safe /enqueue with atomic claim (supabase-py v2), VTT output.
# Env required: SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY
# Optional: WORKER_TOKEN, SUPABASE_OUTPUTS_BUCKET (default "outputs"), SUPABASE_VIDEOS_BUCKET (info only)
...
def make_dummy_vtt(lesson_id: str | None, job_id: str) -> bytes:
    """Minimal valid VTT so downstream promotion works. Replace with real STT later."""
    lid = (lesson_id or "unknown")[:8]
    jid = job_id[:8]
    lines = [
        "WEBVTT",
...
def enqueue():
    try:
        ...
        # 1) Atomically claim if currently queued — supabase-py v2: DO NOT chain .select() on update
        now = utcnow_iso()
        ...
        # Some versions return data on update, some don't — handle both safely
        rows = getattr(upd, "data", None)
        if not rows:
            # Double-check if it was already claimed in the same moment
        ...
        if not rows:
            # Nothing to claim (wrong id or not queued)
        ...
        # 3) Upload to outputs bucket as <job_id>.vtt (idempotent with upsert)
        vtt_key = f"{job_id}.vtt"
        up = supa.storage.from_(OUTPUTS_BUCKET).upload(
            vtt_key, vtt_bytes,
            {"contentType": "text/vtt", "upsert": True}
        ...
        # 4) Create a signed URL (7 days) and store it
        signed = supa.storage.from_(OUTPUTS_BUCKET).create_signed_url(vtt_key, 7 * 24 * 3600)
        vtt_url = signed.get("signedURL") if isinstance(signed, dict) else getattr(signed, "signed_url", None)
        if not vtt_url:
            supa.table("transcription_jobs").update({
